...
// copyPEnumNkCollapseStatesBytes copies the data from Go slice as *C.enum_nk_collapse_states.
func copyPEnumNkCollapseStatesBytes(slice *sliceHeader) (*C.enum_nk_collapse_states, *cgoAllocMap) {
	allocs := new(cgoAllocMap)
	defer runtime.SetFinalizer(allocs, func(a *cgoAllocMap) {
		go a.Free()
	})
	...
	mem0 := unsafe.Pointer(C.CBytes(*(*[]byte)(unsafe.Pointer(&sliceHeader{
		Data: slice.Data,
		Len:  int(sizeOfEnumNkCollapseStatesValue) * slice.Len,
		Cap:  int(sizeOfEnumNkCollapseStatesValue) * slice.Len,
	}))))
	allocs.Add(mem0)

	return (*C.enum_nk_collapse_states)(mem0), allocs
}

// allocEnumNkCollapseStatesMemory allocates memory for type C.enum_nk_collapse_states in C.
// The caller is responsible for freeing the this memory via C.free.
func allocEnumNkCollapseStatesMemory(n int) unsafe.Pointer {
	mem, err := C.calloc(C.size_t(n), (C.size_t)(sizeOfEnumNkCollapseStatesValue))
	if mem == nil {
		panic(fmt.Sprintln("memory alloc error: ", err))
	}
	return mem
...
const sizeOfEnumNkCollapseStatesValue = unsafe.Sizeof([1]C.enum_nk_collapse_states{})
